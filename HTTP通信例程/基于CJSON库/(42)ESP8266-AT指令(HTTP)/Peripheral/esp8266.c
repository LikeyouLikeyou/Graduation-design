...
{
	ESP8266_GPIO_Config ();                //复位引脚配置
	
	ESP8266_USART_Config ();               //串口3配置
...
static void ESP8266_USART_Config ( void )
{
	...
	/* USART1 mode config */
	USART_InitStructure.USART_BaudRate = macESP8266_USART_BAUD_RATE;        //波特率115200
...
/*
  *****************************************************************************
  * @Name   : ESP8266_USART_NVIC_Configuration
  *
  * @Brief  : 串口3中断优先级配置
  *
  * @Input  : none
...
/*
 * 函数名：ESP8266_ExitLink
 * 描述  ：关闭 TCP/UDP/SSL 传输,这里仅适用于单连接
 * 输入  ：需要关闭的连接ID号，范围0-5
 *         当ID为5 时，关闭所有连接。（开启 server后ID为5无效）
 * 返回  : 无
 * 调用  ：被外部调用
 */
void ESP8266_ExitLink(void)
{
	delay_ms ( 500 );                         //这里延时500ms是为了搭配透出透传指令
	ESP8266_Cmd("AT+CIPCLOSE","OK",0,500);
}
