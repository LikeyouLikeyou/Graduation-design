...
u8 FindStr(char* dest,char* src,u16 retry_nms)
{
	...
	while(strstr(dest,src)==0 && retry_nms--)     //等待串口接收完毕或超时退出，一开始这里成立，当strstr查到目标字符串便会返回地址使while条件不成立跳出
    {		
		delay_ms(10);
    }
	
		if(retry_nms)     return 1;                  //当上面   retry_nms减到0时跳出while循环这里的retry_nms值为0xFFFF                
